...
class FlaskrTestCase(unittest.TestCase):
    ...
    def test_login(self):
        rv = self.apps.post("/adduser",data=self.get_user_data_1(),follow_redirects=True)
        assert "user added" in rv.data
        data_out = json.dumps({"username":"test1","pasword":"test1"})
        rv = self.apps.post("/login", data=data_out, follow_redirects=True)
        assert "1" in rv.data
        data_out = json.dumps({"username":"feluser","pasword":"test1"})
        rv = self.apps.post("/login", data=data_out, follow_redirects=True)
        assert "usernameError" in rv.data
        data_out = json.dumps({"username":"test1","pasword":"felpass"})
        rv = self.apps.post("/login", data=data_out, follow_redirects=True)
    ...
    def get_user_pas_1(self):
        username = "test1"
        pasword = "test1"
        return json.dumps({"username":username,"pasword":pasword})
    ...
    def get_post_data_1(self):
        ...
        return json.dumps({"username":username,"pasword":pasword,"name":name,"description":desc,"position_list":positions})
    ...
    def get_comment_data_1(self):
        ...
        return json.dumps({"id_p":id_p,"username":username,"pasword":pasword,"comment":comment})

    def get_friend_data_1(self):
        username = "test1"
        pasword = "test1"
        friend = 2
        return json.dumps({"username":username,"pasword":pasword,"id_u_friend":friend})
